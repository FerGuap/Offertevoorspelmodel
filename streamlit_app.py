...
with col3:
    date_range = st.date_input("Periode", [df["Aanmaakdatum"].min(), df["Aanmaakdatum"].max()])

mask = (
    df["Verkoper"].isin(selected_verkoper) &
    df["klant"].isin(selected_klant) &
    df["Aanmaakdatum"].between(pd.to_datetime(date_range[0]), pd.to_datetime(date_range[1]))
)
filtered_df = df[mask]
